...
const AdminProject = () => {
  ...
  const handleEdit = (Id) => {
    ...
    setAddProject({
      ...
      LiveURL: EditingPro[0].LiveURL || "",
      RepoURL: EditingPro[0].RepoURL || "",
    });
    setPopUp(true);
  };
  const handleCancel = () => {
  ...
  return (
    <div className="relative">
      <div className="flex justify-end ">
        <button
          className="px-4 sm:px-6 py-1.5 bg-primary rounded-md text-white text-lg font-medium hover:scale-105 mb-6"
      ...
      <div className="grid  grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 items-center w-full">
        {ProjectData?.map((Data, index) => {
          return (
            <div
              className="p-6 border-2 flex flex-col gap-2 text-xl min-w-0 overflow-hidden"
              key={index}
            >
              ...
              <div className="flex flex-wrap justify-end gap-3 mt-6">
                <button
                  className="px-4 sm:px-6 py-1.5 bg-primary-red rounded-md text-white text-lg font-medium hover:scale-105 hover"
                  onClick={() => handleDelete(Data?._id)}
                >
                  Delete
                </button>
                <button
                  className="px-4 sm:px-6 py-1.5 bg-primary rounded-md text-white text-lg font-medium hover:scale-105 hover"
                  onClick={() => {
                    handleEdit(Data?._id);
                    setEdit(true);
                  }}
        ...
        <div className="fixed inset-0 flex flex-col bg-opacity-50 bg-black z-100 items-center justify-center ">
          <div className="rounded-md p-6 sm:w-[550px] bg-white">
            ...
            <InputField
              ...
              name={"RepoURL"}
              value={addProject.RepoURL}
              placeholder={"https://github.com/you/repo"}
              onChange={handleChange}
            />
            <div className="flex flex-wrap justify-end gap-3 mt-6">
              <button
                className="px-4 sm:px-6 py-1.5 hover:bg-primary-red hover:text-white rounded-md bg-white text-primary-red border border-primary-red text-lg font-medium hover:scale-105 "
                onClick={() => {
              ...
              <button
                className="px-4 sm:px-6 py-1.5 hover:bg-primary rounded-md hover:text-white border border-primary text-primary bg-white text-lg font-medium hover:scale-105 hover"
